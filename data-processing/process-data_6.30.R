...
for(x in all.files){
  data <- read.csv(paste(input.folder,x,sep="/"))
  nrows = (1:nrow(data))
  for(i in nrows){if((data$rt[i] == "null" && data$task[i] == "respond") == TRUE){data$rt[i] <- data$rt[i+1]}}
  for(p in nrows){if(data$task[p] == "respond" && data$key_press[p] == "null"){data$key_press[p] <- data$key_press[p+1]}}
  write.csv(data, file = paste0(input.folder, "/corr_",x))}
